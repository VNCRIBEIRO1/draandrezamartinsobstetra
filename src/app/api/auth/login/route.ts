...
// Credentials — MUST be configured in .env.local or Vercel Environment Variables
const ADMIN_EMAIL = process.env.ADMIN_EMAIL;
const ADMIN_PASSWORD = process.env.ADMIN_PASSWORD;
const SECRETARY_EMAIL = process.env.SECRETARY_EMAIL;
const SECRETARY_PASSWORD = process.env.SECRETARY_PASSWORD;
const JWT_SECRET = process.env.JWT_SECRET;

// Fail fast if critical env vars are missing
if (!JWT_SECRET) {
  console.error('[SECURITY] JWT_SECRET não configurado. Configure em .env.local ou Vercel.');
}
if (!ADMIN_EMAIL || !ADMIN_PASSWORD) {
  console.error('[SECURITY] Credenciais admin não configuradas. Configure em .env.local ou Vercel.');
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Reject login if credentials are not configured
