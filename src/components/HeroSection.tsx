...
export default function HeroSection() {
  return (
    <section className="relative min-h-screen flex items-center bg-gradient-to-br from-baby-sage via-baby-cream to-baby-beige overflow-hidden">
      ...
      {/* Decorative blobs */}
      <div className="absolute top-20 right-10 w-72 h-72 bg-secondary-300/20 rounded-full blur-3xl animate-pulse-soft" />
      <div className="absolute bottom-20 left-10 w-96 h-96 bg-sage-200/20 rounded-full blur-3xl animate-pulse-soft" />
      <div className="absolute top-1/2 left-1/3 w-64 h-64 bg-sage-200/20 rounded-full blur-3xl" />
        ...
        <div className="grid lg:grid-cols-2 gap-12 items-center">
          ...
          >
            ...
            <h1 className="text-4xl md:text-5xl lg:text-6xl font-serif font-bold text-gray-900 leading-tight mb-6">
              ...
              <span className="text-gold-500">Fases da Vida</span>
            </h1>
            ...
            {[
              ...
              <motion.div
                ...
                className={`backdrop-blur-sm rounded-2xl p-5 border transition-all duration-300 hover:shadow-lg ${
                  item.highlight
                    ? 'bg-secondary-50/80 border-secondary-200 hover:border-secondary-300 ring-1 ring-secondary-100'
                    : 'bg-white/80 border-primary-200 hover:border-secondary-300'
                }`}
              >
                <div className="flex items-start gap-4">
                  <div className={`w-11 h-11 rounded-xl flex items-center justify-center flex-shrink-0 ${
                    item.highlight ? 'bg-secondary-100' : 'bg-primary-100'
                  }`}>
                    <item.icon className={`w-5 h-5 ${item.highlight ? 'text-secondary-600' : 'text-secondary-600'}`} />
                  </div>
                  <div>
